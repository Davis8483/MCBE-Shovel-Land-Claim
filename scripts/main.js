...
import { world, system, ItemStack, EntityRidingComponent, EntityRideableComponent, BlockComponentTypes, EntityComponentTypes, } from '@minecraft/server';
import { database, PlayerData, Claim, PermissionTypes, settings } from './database.js';
import { playSound, AddonSounds } from './sounds.js';
import { sendNotification } from './notifications.js';
import { ShovelUI } from './shovel_ui.js';
var shovelID = "slc:claim_shovel";
/**
...
function runInAllClaims(callback) {
    ...
    try {
        for (var database_1 = __values(database), database_1_1 = database_1.next(); !database_1_1.done; database_1_1 = database_1.next()) {
            ...
            try {
                for (var claims_1 = (e_2 = void 0, __values(claims)), claims_1_1 = claims_1.next(); !claims_1_1.done; claims_1_1 = claims_1.next()) {
                ...
                try {
                    if (claims_1_1 && !claims_1_1.done && (_b = claims_1.return)) _b.call(claims_1);
                }
...
function getClosestPlayer(blockLocation) {
    ...
    try {
        ...
        for (var _b = __values(world.getAllPlayers()), _c = _b.next(); !_c.done; _c = _b.next()) {
            var p = _c.value;
                ...
                // if player is crouching
                    ...
                    if (playerData.resizingClaimName.length > 0) {
                        ...
                        try {
                            ...
                            for (var _c = __values(world.getAllPlayers()), _d = _c.next(); !_d.done; _d = _c.next()) {
                                var p = _d.value;
                    ...
                    // not resizing, create a new claim
                        ...
                        try {
                            // make sure another player isn't in the area
                            for (var _e = __values(world.getAllPlayers()), _f = _e.next(); !_f.done; _f = _e.next()) {
                                var p = _f.value;
                                // we are creating a claim object just to use the isOverlap utility, this is not saved to the database
...
world.afterEvents.pistonActivate.subscribe(function (data) {
    ...
    if (data.dimension == world.getDimension("overworld") && (data.piston.getAttachedBlocks().length > 0)) {
        ...
        try {
            ...
            for (var _c = __values(data.piston.getAttachedBlocks()), _d = _c.next(); !_d.done; _d = _c.next()) {
                var block = _d.value;
                if (data.isExpanding) {
        ...
        if (!allowed) {
            try {
                for (var _e = __values(data.piston.getAttachedBlocks().reverse()), _f = _e.next(); !_f.done; _f = _e.next()) {
                    var block = _f.value;
...
system.runInterval(function () {
    var e_9, _a, e_10, _b;
    try {
        for (var _c = __values(world.getDimension("overworld").getEntities()), _d = _c.next(); !_d.done; _d = _c.next()) {
            var e = _d.value;
            ...
            runInAllClaims(function (playerID, playerName, claim) {
                if (e.isValid() && claim.isOverlap(e.location, e.location)) {
                    ...
                    if (e.getDynamicPropertyIds().includes("entranceVelocity") && e.typeId == "minecraft:creeper") {
                        var xVelocity = e.getDynamicProperty("entranceVelocity").x;
                        var zVelocity = e.getDynamicProperty("entranceVelocity").z;
                        // eject from claim
...
system.runInterval(function () {
    ...
    runInAllClaims(function (playerID, playerName, claim) {
        ...
        try {
            ...
            for (var _c = __values(world.getAllPlayers()), _d = _c.next(); !_d.done; _d = _c.next()) {
                var p = _d.value;
                if (p.id == playerID && ((_b = p.getComponent(EntityComponentTypes.Inventory).container.getItem(p.selectedSlotIndex)) === null || _b === void 0 ? void 0 : _b.matches(shovelID))) {
                    // set flag
                    claimShovelOut = true;
                    break;
                }
